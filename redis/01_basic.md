...
## Stream
레디스를 메세지 브로커로 활용하기 위한 자료 구조다.  
카프카와 유사하게 Consumer-Group 개념을 도입했다.  
이를 통해서 데이터를 분산 처리 할 수 있다.

Redis Stream은 `append-only` 방식으로 데이터를 저장하여 실시간 이벤트 처리나 로그성 데이터의 저장을 위해 사용할 수 있다.